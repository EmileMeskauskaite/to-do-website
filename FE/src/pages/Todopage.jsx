...
export default function ToDoPage() {
  ...
  // Handle task creation with selected status
  const handleSubmit = async (e) => {
    ...
    const response = await fetch('http://localhost:3000/create_todo', {
      ...
      body: JSON.stringify({ 
        ...
        description, 
        userId,
        status,  // Use selected status from dropdown
      }),
    });
    ...
    if (response.ok) {
      ...
      settask('');
      setDescription('');
      setStatus('In Progress'); // Reset to default status after submission
    } else {
  ...
  const updateTaskStatus = async (taskId, newStatus) => {
    const response = await fetch(`http://localhost:3000/update_task_status/${taskId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      ...
      <div className="row">
        ...
        <div className="col-md-6">
          ...
          {isLoading ? (
            ...
            <ToDoList tasks={tasks} updateTaskStatus={updateTaskStatus} getToDoList={getToDoList} /> // Pass getToDoList function
          )}
        </div>
      </div>
